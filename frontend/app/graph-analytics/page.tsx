...
export default function GraphAnalysis() {
  ...
  const today = new Date(); // replace with `new Date()` for live date
  ...
  const fetchAppStats = async ()=>{
    try{
      ...
      const aggregatedMap = new Map<number, AppData>();
      rawData.forEach((item: any) => {
        const uaId = item.as_ua_id;
        const appName = item.userApp?.app?.app_name || "Unknown";

        if (aggregatedMap.has(uaId)) {
          // Sum the count
          aggregatedMap.get(uaId)!.as_count += item.as_count;
        } else {
          // Add new entry with appName
          aggregatedMap.set(uaId, {
            ...item,
            appName, // attach appName directly
          });
        }
      });

const processedData: AppData[] = Array.from(aggregatedMap.values());
